...
.PHONY: show_fuses
show_fuses:
	@echo required fuses for are: ${FUSES}
	avrdude -c ${PROGRAMMER} -P ${PORT} -p ${PARTNO}  ${OPTIONS} -U lfuse:r:"Fuses.hex":h -U hfuse:r:"HFuse.hex":h  -U efuse:r:"EFuse.hex":h
	@type HFuse.hex >> Fuses.hex
	@type EFuse.hex >> Fuses.hex
	@echo found fuse settings are:
	@type Fuses.hex
	@del Fuses.hex HFuse.hex EFuse.hex
